...
va2pa(Pde *pgdir, u_long va)
{
    ...
    if (!(*pgdir & PTE_V))
    {
        //printf("first va2pa if\n");
        return ~0;
    }
    p = (Pte *)KADDR(PTE_ADDR(*pgdir));
    if (!(p[PTX(va)] & PTE_V))
    {
        //printf("second va2pa if\n");
        return ~0;
    }
...
void set_physic_mm()
{
     ...
     */
    maxpa = 250 * 1024 * 1024; // Set maximum physical address to 64MB
    basemem = maxpa;          // Amount of base memory equals to 64MB
...
void vm_init()
{
    ...
    //printf("The page directory physical address: %x \n", va2pa(pgdir, pgdir));
    mCONTEXT = (int)pgdir;
    ...
    //printf("The stauct page physical address: %x \n", va2pa(pgdir, pages));
...
int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
    ...
    //printf("walk page table pointer: %x\n", pgtable);
    /* Step 3: Set the page table entry to `*ppte` as return value. */
    if (ppte)
    {
        *ppte = pgtable + PTX(va);
    }
    //printf("walk page table entry: %x\n", ppte);
    return 0;
}
...
int page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
    ...
    if (pgtable_entry != 0 && (*pgtable_entry & PTE_V) != 0)
    {
        ...
        {
            tlb_invalidate(pgdir, va);        //这里TLB更新的逻辑无法理解，所以重构
...
void tlb_refill_test()
{
    // uint32_t * addr = 0x10000000;
    
    // for(uint16_t i = 0; i < 1024; i++)
    // {
    //     addr[i] = (uint32_t)0x12345678;    
    //     printf("va:0x1000_0000 + %u, val: %x \n", i, addr[i]);
    // }
    
    ulong *addr;
    addr = 0x10000000;
    printf("***********Start to TLB test***********\n");
    
    // pageout(0x10000000, 0x80400000);
    printf("!!!!!!!!\n");
